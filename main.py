...
class DatabaseConnection:
	def __init__(self, host="localhost", user="root", password="Dallas123_!", database="OnlineShop"):
		self.host = host
		self.user = user
...
class InsertDialog(QDialog):
	...
	def add_customer(self):
		...
		connection = DatabaseConnection().connect()
		cursor = connection.cursor()
		cursor.execute("INSERT INTO customers (name, country, mobile) VALUES (%s,%s,%s)",
					   (name, country, mobile))
		connection.commit()
		cursor.close()
		connection.close()
...
class EditDialog(QDialog):
	...
	def update_customer(self):
		connection = DatabaseConnection().connect()
		cursor = connection.cursor()
		cursor.execute("UPDATE customers SET name = %s, country = %s, mobile = %s WHERE id =%s",
